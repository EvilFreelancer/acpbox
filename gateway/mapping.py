...
def acp_run_output_to_chat_completion(
    ...
    """Build OpenAI chat.completion response from ACP Run."""
    ...
    choice = ChatCompletionChoice(
        index=0,
        message=ChatCompletionChoiceMessage(role="assistant", content=content or None),
        finish_reason="stop",
    )
    created = int(time.time())
    return CreateChatCompletionResponse(
        id=run_id or run.get("run_id") or f"chatcmpl-{uuid.uuid4().hex[:24]}",
        created=created,
        model=model,
        choices=[choice],
...
def acp_run_output_to_response_body(
    ...
    """Build OpenAI response object from ACP Run."""
    output = run.get("output") or []
    text_parts: list[str] = []
    for msg in output:
        if not isinstance(msg, dict):
    ...
    out_message = ResponseOutputMessage(
        ...
        content=[ResponseOutputMessageContent(type="output_text", text=text, annotations=[])],
    )
